 ...
 * 19.  First Run
 * 20.  Match Fitness Function (0 -> Naive, 1 -> LCS)
 * 21.  LCS Fitness SDA Output Percentage
 *
 * @param argc
 * @param argv
 * @return
...
int main(int argc, char *argv[]) {
    ...
    SDA expBestSDA = SDA(sdaStates, numChars, 2, maxSeqLen);
    ...
    sprintf(pathToOut, "./Output/TripleA on Set%d with %.1fmilMMEs, %04dPS, %02dSt, %02dNTM, %02dNRM, %s,"
                       " %dTS, %sCO, %03d%%CrR, %03d%%MR, %03d%%CuR, %sCu, %dCE, %dFit, %.2fLCS/", seqSet,
            (double) maxGens / 1000000, popsize, sdaStates, initNumTransMuts, initNumRespMuts, dynamicMessage,
            tournSize, (crossoverOp == 0 ? "2Pt" : "1St"), (int) (crossoverRate * 100), (int) (mutationRate * 100),
            (int) (cullingRate * 100), (randomCulling ? "Rand" : "Worst"), CULLING_EVERY, matchFitnessFunc,
            pctgAvgLen);
    ...
    for (int run = initRun; run < initRun + runs; ++run) {
